            ...
            <div class="row align-items-center justify-content-between feature_area" style="margin-bottom: 60px;">
               <div class="col-sm-3 text-center border-right" data-aos="zoom-in" data-aos-easing="ease-in-back" data-aos-duration="1000">
                  ...
                  @endif
                   ...
                   @if(Auth::guard('gamer')->check() )
                  @if($tournaments->user_type==2)
                  @if(Auth::guard('gamer')->user()->gamer_type == 1)
                  <a href="https://letsgamenow.com/gamer/profile/{{Auth::guard('gamer')->user()->id}}" class="red__btn">JOIN</a>
                  <p style="color: #fff">You need to create a team to join this tournament!</p>
                  @else
                  <a  data-toggle="modal" data-target="#jointurnament_modal" href="#jointurnament_modal" class="red__btn">JOIN</a> 
                  @endif
                  @else
               ...
               <div class="col-sm-3 text-center border-right" data-aos="zoom-in" data-aos-easing="ease-in-back" data-aos-duration="1000">
                  ...
                  @else
                  <a href="{{route('team_fixture',request()->route()->id)}}" class="white__btn">FIXTURE</a>
                  @endif
               </div>
               @endif
         ...
         <div class="modal-body">
            ...
            <form method="post" action="{{route('gameringameidsave')}}">
               ...
               <div class="upadate_field add_game_field mb-3">
                  <input class="form-control" placeholder="In Game Name" type="text" name="Ingame Name" id="ingame_name" required>
                  <div style="display: none; color: #fff;" id="ingame_name_error" class="validation-advice">This is a required field.</div>
               </div>
               <div class="upadate_field add_game_field mb-3">
...
{{--                  <input class="form-control" placeholder="In Game Id" type="text" name="Ingame Id" id="ingame_id">--}}
{{--               </div>--}}
{{--               @elseif($tournaments->game_id==1 || $tournaments->game_id==18 ||$tournaments->game_id==17 ||$tournaments->game_id==5 ||$tournaments->game_id==4)--}}
{{--               <div class="upadate_field add_game_field mb-3">--}}
...
<script>
    $(function () {
        ...
        $(document).on("click", "#joinGame", function () {
           ...
           if(ingame_name == '') {
              $('#ingame_name_error').show();
              return false;
           }else {
              $('#ingame_name_error').hide();
           }
