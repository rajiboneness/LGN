...
@section("content")
   <section class="banner__area game inner">
      <img src="{!!asset('new-theme\images\site banner\Tournaments.jpg')!!}
      " class="img-fluid">
      ...
      <div class="explore__tab mt-3 mb-3">
        <div class="container">
          <div class="navbar-nav flex-row justify-content-center filter-option">
            ...
            ?>
            <a class="nav-item nav-link <?php if($url_pos==0){echo "active";} ?>" href="{!! URL::to('tournament') !!}" data-category="all">All</a>
            @foreach($games as $game)
            <?php $key1 = strtolower(preg_replace("/[^a-zA-Z0-9]+/", "-", $game->name)); ?>
            <a class="nav-item nav-link <?php if($url_pos==$game->id){echo "active";} ?>" href="{!! URL::to('tournament-sorting/'.$game->id.'/'.$key1) !!}" data-category="game{{$game->id}}">{{$game->name}}</a>
            @endforeach
          </div>
        </div>
      </div>
              ...
              <div class="news__blocks__footer row m-0">
                ...
                <div class="col-6 col-sm-3">
                  <p>Max Player</p>
                   <h4>
                    @if($data->user_type==1)
                    @if(isset($gamerplayerjoined))
                    @foreach($gamerplayerjoined as $playerjoined)
                    @if($playerjoined->tournament_id == $data->id)
                    {{$playerjoined->count}} 
                    @endif
                    @endforeach  
                    @endif                  
                    @else($data->user_type==2)
                    @if(isset($teamsplayerjoined))
                    @foreach($teamsplayerjoined as $playerjoined)
                    @if($playerjoined->tournament_id == $data->id)
                    {{$playerjoined->count}} 
                    @endif
                    @endforeach
