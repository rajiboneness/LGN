...
@section("content")
    ...
    <section class="sponser_bg pt-5 pb-5">
      ...
      <div class="explore__tab mt-3 mb-3">
        <div class="container">
          <div class="navbar-nav flex-row justify-content-center filter-option">
            ...
            ?>
            <a class="nav-item nav-link <?php if($url_pos==0){echo "active";} ?>" href="{!! URL::to('leaderboards') !!}" data-category="all">All</a>
            @foreach($games as $game)
            <?php $key1 = strtolower(preg_replace("/[^a-zA-Z0-9]+/", "-", $game->name)); ?>
            <a class="nav-item nav-link <?php if($url_pos==$game->id){echo "active";} ?>" href="{!! URL::to('game-wise-leaderboard/'.$game->id.'/'.$key1) !!}" data-category="game{{$game->id}}">{{$game->name}}</a>
            @endforeach
          </div>
        </div>
      </div>
      ...
      <div class="container">
        <h5 class="leaderboard__name">{{$game_name}}</h5>
        ...
        <?php $slno = 1; ?>
          ...
          <div class="table__row thead" data-aos="fade-up" data-aos-easing="ease-in-back" data-aos-duration="1000">
            ...
            <div class="table__col text-center">{{$leader->user_point}}</div>
            <div class="table__col text-right">{{$leader->name}}</div>
          </div>
          <?php $slno = $slno + 1; ?>
         @endforeach
         @endif
